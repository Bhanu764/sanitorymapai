...
// Provide a simple window.storage fallback backed by localStorage if not provided by host
if (!window.storage) {
  window.storage = {
    // list(prefix) -> { keys: [string] }
    async list(prefix = 'report:') {
      const keys = []
...
const App = () => {
    ...
    try {
      ...
      if (keys && keys.keys) {
        ...
        for (const key of keys.keys) {
          ...
          if (result) {
            try {
              loadedReports.push(JSON.parse(result.value))
            } catch (e) {
              // ignore malformed
            }
          }
        ...
        // sort newest first
        loadedReports.sort((a, b) => new Date(b.date) - new Date(a.date))
  ...
  const classifySeverity = (description) => {
    ...
    if (highCount > 0) return { level: 'high', color: '#ef4444' }
    if (mediumCount > 0) return { level: 'medium', color: '#eab308' }
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      await window.storage.set(`report:${newReport.id}`, JSON.stringify(newReport))
  ...
  const updateReportStatus = async (reportId, newStatus) => {
    try {
      ...
      if (report) {
        ...
        await window.storage.set(`report:${reportId}`, JSON.stringify(updatedReport))
  ...
  // Local maps key from env var (Vite uses import.meta.env)
  const mapsKey = import.meta.env.VITE_GOOGLE_MAPS_KEY || ''
  // Compute iframe src: prefer Google Maps Embed when key provided,
  // otherwise fall back to an OpenStreetMap embed so the map is visible
  const delta = 0.05
  const osmBBox = `${mapCenter.lng - delta}%2C${mapCenter.lat - delta}%2C${mapCenter.lng + delta}%2C${mapCenter.lat + delta}`
  const iframeSrc = mapsKey
    ? `https://www.google.com/maps/embed/v1/view?key=${mapsKey}&center=${mapCenter.lat},${mapCenter.lng}&zoom=${mapZoom}&maptype=roadmap`
    : `https://www.openstreetmap.org/export/embed.html?bbox=${osmBBox}&layer=mapnik&marker=${mapCenter.lat}%2C${mapCenter.lng}`
              ...
              <div className="relative" style={{ height: '600px' }}>
                ...
                <div className="absolute top-4 left-4 right-4 pointer-events-none">
                  <div className="bg-white rounded-lg shadow-lg p-4 pointer-events-auto">
                    <p className="text-sm text-gray-600 mb-2">Click on a report below to view its location on the map</p>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    {reports.map((report) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className="w-4 h-4 rounded-full mt-1 flex-shrink-0" style={{ backgroundColor: report.color }} />
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-gray-800 truncate">{report.userName}</p>
            ...
            <div className="mt-6 bg-white rounded-lg shadow-lg p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-5 h-5 rounded-full" style={{ backgroundColor: selectedReport.color }} />
                    <h3 className="text-xl font-bold text-gray-800 capitalize">{selectedReport.severity} Priority</h3>
                  </div>
                  <p className="text-gray-600">Reported by: {selectedReport.userName}</p>
                  <p className="text-sm text-gray-500">Date: {new Date(selectedReport.date).toLocaleDateString()}</p>
                </div>
              ...
              {selectedReport.image && (
                <div className="mb-4">
                  ...
                  <img src={selectedReport.image} alt="Issue" className="max-h-64 rounded-lg" />
                </div>
              )}
              ...
              <div className="mb-4">
                ...
                <p className="text-gray-700">
                  Latitude: {selectedReport.location.lat.toFixed(6)}, Longitude: {selectedReport.location.lng.toFixed(6)}
                </p>
              </div>
